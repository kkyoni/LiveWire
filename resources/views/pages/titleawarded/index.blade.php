...
@section('title')
    {{ __('messages.Titleawarded Management') }}
@endsection
@section('content')
    <div class="col-xl-12">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('messages.Titleawarded Table') }}</h3>
                <div class="block-options">
                    @if (\App\Helpers\Helper::checkPermission(['titleawarded.create']))
                        <div class="block-options-item">
                            <a href="{{ route('titleawarded.create') }}"
                                class="btn btn-primary btn-sm">{{ __('messages.Add Titleawarded') }}</a>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">{{ __('messages.Titleawarded Management Detail') }}</h3>
                        <div class="block-options">
...
@section('scripts')
    <script>
        $(document).on("click", "a.deletetitleawarded", function(e) {
            var row = $(this);
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ route('titleawarded.delete', ['']) }}" + "/" + id,
                        type: 'post',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
                    ...
                    swal("Abgesagt", "Ihre verliehenen Titel sind sicher :)", "error");
                }
            });
            return false;
        })
        $(document).on("click", "a.showtitleawarded", function(e) {
            var row = $(this);
            var id = $(this).attr('data-id');
            $.ajax({
                url: "{{ route('titleawarded.show') }}",
                type: 'get',
                data: {
                    id: id
                },
