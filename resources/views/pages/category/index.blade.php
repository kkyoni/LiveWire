    ...
    <div class="col-xl-12">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('messages.Category Table') }}</h3>
                <div class="block-options">
                    @if (\App\Helpers\Helper::checkPermission(['category.create']))
                        <div class="block-options-item">
                            <a href="{{ route('category.create') }}"
                                class="btn btn-primary btn-sm">{{ __('messages.Add Category') }}</a>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">{{ __('messages.Category Management Detail') }}</h3>
                        <div class="block-options">
        ...
        $(document).on("click", "a.deletecategory", function(e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    swal("Abgesagt", "Ihre Kategorie ist sicher :)", "error");
                }
            });
            return false;
        })
