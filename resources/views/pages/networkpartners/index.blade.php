...
@section('title')
    {{ __('messages.Network Partners Management') }}
@endsection
@section('content')
    <div class="col-xl-12">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title"> {{ __('messages.Network Partners Table') }}</h3>
                <div class="block-options">
                    @if (\App\Helpers\Helper::checkPermission(['networkpartners.create']))
                        <div class="block-options-item">
                            <a href="{{ route('networkpartners.create') }}"
                                class="btn btn-primary btn-sm">{{ __('messages.Add Network Partners') }}</a>
                        </div>
                    @endif
                </div>
            </div>
                        ...
                        <div class="col-lg-12 col-xl-12">
                            <div class="col-sm-12">
                                <div class="form-group row">
                                    <?php
                                    $networkpartner = App\Models\NetworkPartner::get();
                                    ?>
                                    <div class="col-sm-3 col-sm-offset-3">
                                        <select id='bezeichnung' class="js-select2 form-select form-control">
                                            <option value=''>{{ __('messages.Select Bezeichnung') }}</option>
                                            <?php
                                            foreach ($networkpartner as $networkpartner_list) {
                                                if (!empty($networkpartner_list->bezeichnung)) {
                                                    echo "<option value='$networkpartner_list->bezeichnung'>$networkpartner_list->bezeichnung</option>";
                                                }
                                            } ?>
                                        </select>
                                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title"> {{ __('messages.Network Partners Management Detail') }}</h3>
                        <div class="block-options">
...
@section('styles')
    <style>
        <?php
    $checkPermission = \App\Helpers\Helper::checkPermission(['networkpartners.export']);
...
@section('scripts')
    ...
    <script type="text/javascript">
        $(function() {
            ...
            var table = $('.data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('networkpartners.index') }}",
                    data: function(d) {
    ...
    <script>
        $(document).on("click", "a.deletenetworkpartners", function(e) {
            var row = $(this);
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ route('networkpartners.delete', ['']) }}" + "/" + id,
                        type: 'post',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
                    ...
                    swal("Abgesagt", "Ihre Netzwerkpartner sind sicher :)", "error");
                }
            });
            return false;
        })
        $(document).on("click", "a.shownetworkpartners", function(e) {
            var row = $(this);
            var id = $(this).attr('data-id');
            $.ajax({
                url: "{{ route('networkpartners.show') }}",
                type: 'get',
                data: {
                    id: id
                },
