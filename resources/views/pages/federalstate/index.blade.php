...
@section('title')
    {{ __('messages.federalstate Management') }}
@endsection
@section('content')
    <div class="col-xl-12">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('messages.federalstate Table') }}</h3>
                <div class="block-options">
                    @if (\App\Helpers\Helper::checkPermission(['federalstate.create']))
                        <div class="block-options-item">
                            <a href="{{ route('federalstate.create') }}"
                                class="btn btn-primary btn-sm">{{ __('messages.Add federalstate') }}</a>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">{{ __('messages.federalstate Management Detail') }}</h3>
                        <div class="block-options">
...
@section('scripts')
    <script>
        $(document).on("click", "a.deletefederalstate", function(e) {
            var row = $(this);
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ route('federalstate.delete', ['']) }}" + "/" + id,
                        type: 'post',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
                    ...
                    swal("Abgesagt", "Eure Bundesländer sind sicher :)", "error");
                }
            });
            return false;
        })
        $(document).on("click", "a.showfederalstate", function(e) {
            var row = $(this);
            var id = $(this).attr('data-id');
            $.ajax({
                url: "{{ route('federalstate.show') }}",
                type: 'get',
                data: {
                    id: id
                },
