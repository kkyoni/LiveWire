...
@section('title')
    {{ __('messages.Title prefix Management') }}
@endsection
@section('content')
    <div class="col-xl-12">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('messages.Titleprefix Table') }}</h3>
                <div class="block-options">
                    @if (\App\Helpers\Helper::checkPermission(['titleprefix.create']))
                        <div class="block-options-item">
                            <a href="{{ route('titleprefix.create') }}"
                                class="btn btn-primary btn-sm">{{ __('messages.Add Titleprefix') }}</a>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">{{ __('messages.Titleprefix Management Detail') }}</h3>
                        <div class="block-options">
...
@section('scripts')
    <script>
        $(document).on("click", "a.deletetitleprefix", function(e) {
            var row = $(this);
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ route('titleprefix.delete', ['']) }}" + "/" + id,
                        type: 'post',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
                    ...
                    swal("Abgesagt", "Dein Titleprefix ist sicher :)", "error");
                }
            });
            return false;
        })
        $(document).on("click", "a.showtitleprefix", function(e) {
            var row = $(this);
            var id = $(this).attr('data-id');
            $.ajax({
                url: "{{ route('titleprefix.show') }}",
                type: 'get',
                data: {
                    id: id
                },
