...
@section('title')
    {{ __('messages.Status Management') }}
@endsection
@section('content')
    <div class="col-xl-12">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('messages.Status Table') }}</h3>
                <div class="block-options">
                    @if (\App\Helpers\Helper::checkPermission(['status.create']))
                        <div class="block-options-item">
                            <a href="{{ route('status.create') }}"
                                class="btn btn-primary btn-sm">{{ __('messages.Add Status') }}</a>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">{{ __('messages.Status Management Detail') }}</h3>
                        <div class="block-options">
        ...
        $(document).on("click", "a.deletestatus", function(e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    swal("Abgesagt", "Dein Status ist sicher :)", "error");
                }
            });
            return false;
        })
        $(document).on("click", "a.showstatus", function(e) {
            var row = $(this);
            var id = $(this).attr('data-id');
            $.ajax({
