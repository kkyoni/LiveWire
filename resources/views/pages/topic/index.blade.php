...
@section('title')
    {{ __('messages.Topic Management') }}
@endsection
@section('content')
    <div class="col-xl-12">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('messages.Topic Table') }}</h3>
                <div class="block-options">
                    @if (\App\Helpers\Helper::checkPermission(['topic.create']))
                        <div class="block-options-item">
                            <a href="{{ route('topic.create') }}"
                                class="btn btn-primary btn-sm">{{ __('messages.Add Topic') }}</a>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">{{ __('messages.Topic Management Detail') }}</h3>
                        <div class="block-options">
...
@section('scripts')
    <script>
        $(document).on("click", "a.deletetopic", function(e) {
            var row = $(this);
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    swal("Abgesagt", "Dein Thema ist sicher :)", "error");
                }
            });
            return false;
        })
        $(document).on("click", "a.showtopic", function(e) {
            var row = $(this);
            var id = $(this).attr('data-id');
            $.ajax({
                url: "{{ route('topic.show') }}",
                type: 'get',
                data: {
                    id: id
                },
