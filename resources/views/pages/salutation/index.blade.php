...
@section('title')
    {{ __('messages.Salutation Management') }}
@endsection
@section('content')
    <div class="col-xl-12">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('messages.Salutation Table') }}</h3>
                <div class="block-options">
                    @if (\App\Helpers\Helper::checkPermission(['salutation.create']))
                        <div class="block-options-item">
                            <a href="{{ route('salutation.create') }}"
                                class="btn btn-primary btn-sm">{{ __('messages.Add Salutation') }}</a>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">{{ __('messages.Salutation Management Detail') }}</h3>
                        <div class="block-options">
...
@section('scripts')
    <script>
        $(document).on("click", "a.deletesalutation", function(e) {
            var row = $(this);
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    swal("Abgesagt", "Deine Anrede ist sicher :)", "error");
                }
            });
            return false;
        })
        $(document).on("click", "a.showsalutation", function(e) {
            var row = $(this);
            var id = $(this).attr('data-id');
            $.ajax({
                url: "{{ route('salutation.show') }}",
                type: 'get',
                data: {
                    id: id
                },
