...
@section('title')
    {{ __('messages.Functions Management') }}
@endsection
@section('content')
    <div class="col-xl-12">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">{{ __('messages.Functions Table') }}</h3>
                <div class="block-options">
                    @if (\App\Helpers\Helper::checkPermission(['functions.create']))
                        <div class="block-options-item">
                            <a href="{{ route('functions.create') }}"
                                class="btn btn-primary btn-sm">{{ __('messages.Add Functions') }}</a>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">{{ __('messages.Functions Management Detail') }}</h3>
                        <div class="block-options">
...
@section('scripts')
    <script>
        $(document).on("click", "a.deletefunctions", function(e) {
            var row = $(this);
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    swal("Abgesagt", "Ihre Funktionen sind sicher :)", "error");
                }
            });
            return false;
        })
        $(document).on("click", "a.showfunctions", function(e) {
            var row = $(this);
            var id = $(this).attr('data-id');
            $.ajax({
                url: "{{ route('functions.show') }}",
                type: 'get',
                data: {
                    id: id
                },
